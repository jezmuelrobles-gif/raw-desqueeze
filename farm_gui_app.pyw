"""Desktop GUI for the warm 'farm' creative grade: pick a folder, it applies
the grade to everything in it automatically. No console window (.pyw)."""
...
class FarmGradeApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        root.title("RAW Desqueeze - Farm Grade")
        ...
        self.log = tk.Text(
            ...
            font=("Consolas", 9), state="disabled", bg="#2b2013", fg="#f0dcb8",
        )
    ...
    def start_batch(self, folder: Path) -> None:
        ...
        thread = threading.Thread(
            ...
            args=(folder, self.overwrite_var.get(), self.recursive_var.get()),
            daemon=True,
        )
        thread.start()

    def _run_batch(self, folder: Path, overwrite: bool, recursive: bool) -> None:
        out_dir = folder / "farm_graded"
        ...
        for i, path in enumerate(supported, 1):
            result = process_file_farm(path, out_dir, SQUEEZE_FACTOR, overwrite)
            if result.status == "success":
                succeeded += 1
                out = f"{result.output_size[0]}x{result.output_size[1]}" if result.output_size else "?"
                self._events.put(("log", f"[{i}/{total}] OK    {path.name}  -> {out}"))
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    messagebox.showinfo("Farm grade complete", f"{summary}\n\nOutput folder:\n{out_dir}")
        except queue.Empty:
            pass
        self.root.after(100, self._poll_events)
